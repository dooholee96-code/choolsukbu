...
def sheet_guide(ws, data, src_name):
    ...
    line(
        "파일정리",
        "업무 파일(모집 서류·예산서·법령·제출자료·검토자료)을 폴더로 어떻게 나누는지. "
        "업무 PC 로는 실행 파일이 못 들어가서, 폴더를 만드는 명령까지 이 워크북에 실었습니다.",
    )
    line(
        "학생현황",
    ...
    line(
        ...
        "'구분'(신규/계속/재유학)이 들어온 쪽이고 '다음 학기'(이어짐/학기말 종료/학기 중 종료)가 나간 쪽입니다. "
        "두 칸을 걸러 보면 신규·기존·종료 유학생이 바로 갈라집니다.",
    )
    line(
        "신청이력",
    ...
    line("학기별집계",
         "학년도·학기별로 '들어온 쪽 → 인원 → 나간 쪽'을 한 줄에 늘어놓은 표입니다. "
    ...
    line("설정",
         "기준일과 체제비 단가. 지침이 바뀌면 노란 칸만 고치면 됩니다. "
         "기준일은 '유학중' 을 어느 날 기준으로 볼지 정합니다 — 다가오는 학기 시작일로 둡니다.")
    line(
        "데이터검증",
    ...
    line(
        "학기가 바뀌면",
        "계속 유학하는 학생은 유학이력에 새 학기 줄을 한 줄씩 더합니다. "
        "줄을 더하면 지난 학기 줄의 '다음 학기'가 '미정'에서 '이어짐'으로 저절로 바뀝니다.",
    )
    line(
        "신규·기존·종료 가려내기",
        "유학이력에서 '학기'로 거른 뒤 ① 신규 유학생 = 구분이 '신규' ② 기존(계속) 유학생 = "
        "구분이 '계속' 또는 '재유학' ③ 그 학기로 끝나는 유학생 = 다음 학기가 '학기말 종료' 또는 "
        "'학기 중 종료' 입니다. 학기별집계의 같은 이름 열이 그 인원을 세어 둔 것입니다.",
    )
    ...
    line(
        ...
        "② 학기말 재적 — 그 학기 끝까지 남은 인원. 학기 중에 그만둔 학생만큼 ①이 ②보다 큽니다. "
        "학기말 재적이 곧 다음 학기 인원은 아닙니다. 학기말까지 다니고 그대로 유학을 마치는 학생이 "
        "있어서, 실제로 넘어가는 인원은 '다음 학기로 이어짐' 열입니다.",
    )
    line(
        "다음 학기 칸 읽는 법",
        "이어짐 = 다음 학기에도 줄이 있는 학생 / 학기말 종료 = 학기를 다 채우고 끝난 학생 / "
        "학기 중 종료 = 학기 도중에 돌아간 학생 / 미정 = 아직 다음 학기 연장 입력 전이라 "
        "이어질지 끝날지 알 수 없는 학생 / 예정 = 아직 시작하지 않은 학기의 줄. "
        "'미정'을 종료로 세면 안 됩니다 — 2026-1학기 325명이 지금 여기에 들어 있습니다.",
    )
    line(
        "대조 결과",
...
ENROLL_HEADERS = [
    ...
    "가구ID", "원 지역", "원 소속교", "학기 상태", "학기말 재적", "다음 학기", "종료 사유",
...
# 업무 PC 로는 bat·exe 가 들어가지 못한다. 워크북(xlsx)은 들어가므로,
# 파일 정리 규칙을 여기에 넣어 워크북과 함께 건너가게 한다.
FOLDER_MAP = [
    ("00_미분류", "아직 자리를 못 정한 파일. 늘 비어 있는 것이 정상입니다. "
                  "새 파일이 어디 갈지 모르겠으면 일단 여기 던져 둡니다."),
    ("10_현황관리", "매일 여는 곳. 명단 원본, 이 워크북, 현황판, 그리고 백업 폴더."),
    ("20_법령지침", "법령 / 도교육청지침 / 타시도참고. 연도로 나누지 않습니다 — "
                    "옛 기준을 계속 들춰 보게 되는 자료라, 한 폴더에 날짜순으로 쌓아야 "
                    "개정 이력이 그대로 보입니다."),
    ("30_모집", "학년도 → 학기 → 단계(1_계획 2_공고서식 3_접수 4_심사배정 5_결과통보). "
                "다음 학기 폴더는 지난 학기를 복사해서 만듭니다. 확정·공식 문서만 둡니다."),
    ("40_예산", "회계연도(1~12월)로 나눕니다. 학년도가 아닙니다. 1_편성 2_집행 3_정산."),
    ("50_제출자료", "밖에 낸 자료. 요구받은 날짜순으로 폴더를 팝니다 — "
                    "260510_서울시교육청_지원금 형식."),
    ("60_발표홍보", "보도자료, 발표자료 PDF, 사진."),
    ("70_검토", "내부 검토용. 제도개선 / 예산검토 / 현황분석 / 질의응답. "
                "아직 확정되지 않은 것, 나만 보는 것이 여기 들어갑니다."),
    ("90_이전담당자", "물려받았지만 아직 안 본 것. 정리된 폴더에 섞지 말고 격리해 두었다가 "
                      "필요할 때 꺼냅니다. 1년째 안 열었으면 압축해서 치웁니다."),
]

MAKE_COMMANDS = [
    r"for %d in (00_미분류 10_현황관리\백업 20_법령지침\법령 20_법령지침\도교육청지침 "
    r"20_법령지침\타시도참고 60_발표홍보 70_검토\제도개선 70_검토\예산검토 "
    r"70_검토\현황분석 70_검토\질의응답 90_이전담당자) do md 농어촌유학\%d",
    r"for %y in (2022 2023 2024 2025 2026) do md 농어촌유학\30_모집\%y학년도 "
    r"농어촌유학\40_예산\%y회계연도 농어촌유학\50_제출자료\%y",
    r"for %y in (2025 2026) do for %t in (1학기 2학기) do "
    r"for %s in (1_계획 2_공고서식 3_접수 4_심사배정 5_결과통보) do "
    r"md 농어촌유학\30_모집\%y학년도\%t\%s",
    r"for %y in (2025 2026) do for %s in (1_편성 2_집행 3_정산) do "
    r"md 농어촌유학\40_예산\%y회계연도\%s",
]


def sheet_files(ws):
    """업무 파일을 어떻게 정리하는지. 워크북에 실어 업무 PC 로 함께 건너간다."""
    ws.sheet_view.showGridLines = False
    ws.column_dimensions["A"].width = 2
    ...
    def head(text):
        ...
        c.fill = PatternFill("solid", fgColor=ACCENT)
        c.alignment = Alignment(vertical="center", indent=1)
        ws.cell(r[0], 3, "").fill = PatternFill("solid", fgColor=ACCENT)
        ws.row_dimensions[r[0]].height = 20
        r[0] += 1

    def line(label, text, mono=False, warn=False):
        a = ws.cell(r[0], 2, label)
        a.font = Font(name=FONT, size=10, bold=True, color="A61C1C" if warn else INK)
        a.alignment = Alignment(vertical="top", indent=1)
        b = ws.cell(r[0], 3, text)
        b.font = Font(name="Consolas" if mono else FONT, size=9 if mono else 10)
        b.alignment = Alignment(vertical="top", wrap_text=True, indent=1)
        if mono:
            b.fill = SUB_FILL
        ws.row_dimensions[r[0]].height = 15 + 13 * (len(text) // 62)
        r[0] += 1
    ...
    title("업무 파일 정리")
    line("왜 여기 있나",
         "업무 PC 로는 bat·exe 같은 실행 파일이 들어가지 못합니다. 이 워크북(xlsx)은 들어가므로 "
         "파일 정리 규칙을 여기에 실었습니다. 아래 명령도 이 시트에서 복사해 쓰시면 됩니다.")
    gap()

    head("어디에 무엇이 있나")
    for name, what in FOLDER_MAP:
        line(name, what)
    gap()

    head("폴더 만드는 법")
    line("① 자리 열기", "폴더를 만들 자리를 탐색기에서 엽니다(바탕화면이면 바탕화면 폴더).")
    line("② cmd 열기",
         "탐색기 주소창에 cmd 를 치고 Enter. 그 폴더에서 명령 프롬프트가 열려 경로를 칠 필요가 없습니다.")
    line("③ 네 줄 실행", "아래 네 칸을 하나씩 복사해 붙여넣고 Enter. 칸을 클릭한 뒤 Ctrl+C 하면 통째로 복사됩니다.")
    for i, cmd in enumerate(MAKE_COMMANDS, start=1):
        line(f"명령 {i}", cmd, mono=True)
    line("안 되면",
         "손으로 만드셔도 됩니다. 꼭 필요한 건 맨 위 아홉 개뿐이고, 하위 폴더는 파일을 넣다가 "
         "필요해질 때 그 자리에서 만들면 됩니다.")
    gap()

    head("파일 이름 짓는 법")
    line("형식", "YYMMDD_구분_내용.확장자")
    line("보기", "260315_공문_2026학년도 2학기 모집 계획 알림.hwp / 260401_대장_2학기 접수현황.xlsx")
    line("날짜", "여섯 자리를 맨 앞에. 이것만으로 탐색기가 시간순 정렬됩니다. 공문 시행일 또는 받은 날.")
    line("구분", "공문 / 서식 / 대장 / 계획 / 제출 / 검토 / 참고 정도면 충분합니다.")
    line("못 쓰는 글자", '\\  /  :  *  ?  "  <  >  |  — 날짜에 2026/03/15 처럼 빗금을 쓰다가 막히는 일이 흔합니다.')
    line("금지", "'최종', '진짜최종', '최종2'. 확정본에만 _확정 을 붙이고 이전 것은 _이전 폴더로 내립니다.",
         warn=True)
    line("개인정보", "파일 이름에 학생 이름을 넣지 마세요.", warn=True)
    gap()

    head("흩어진 파일 정리하는 순서")
    line("1. 쓸어 담기",
         "흩어진 업무 파일을 보지 말고 몽땅 00_미분류 로 옮깁니다. 바탕화면이 깨끗해지는 것만으로 절반입니다.")
    line("2. 쉬운 것부터",
         "00_미분류 를 열고 보기→자세히, 이름순 정렬. 비슷한 이름끼리 붙어 있어 여러 개를 한 번에 옮길 수 있습니다.")
    line("3. 애매한 것",
         "'나중에 이걸 왜 찾게 될까'로 정합니다. 3분 넘게 고민되면 00_미분류 에 그대로 두세요.")
    line("4. 이름은 나중에",
         "옛 파일 이름을 전부 고치려 하지 마세요. 앞으로 만드는 파일부터 규칙을 쓰고, "
         "옛 파일은 찾을 일이 생겼을 때 그때 고칩니다.")
    gap()

    head("이건 어디로?")
    line("학기를 모르겠다", "파일의 '수정한 날짜'를 보세요. 3~5월이면 그해 1학기, 9~11월이면 2학기입니다.")
    line("누가 요구해서 만든 것", "50_제출자료. 만든 날짜로 폴더를 새로 팝니다.")
    line("아직 확정 전인 것", "70_검토. 확정되어 공문이 나가면 그때 30_모집 등으로 옮깁니다.")
    line("물려받은 것", "90_이전담당자. 열어보지 말고 그냥 넣으세요.")
    gap()

    head("명단 파일 다루는 법")
    line("고치는 파일은 하나",
         "10_현황관리\\전북_농어촌유학생_명단.xlsx 하나만 고칩니다. "
         "명단_수정본, 명단_최종 처럼 사본이 늘기 시작하면 어느 것이 진짜인지 모르게 되고, "
         "그 순간부터 숫자를 믿을 수 없습니다.", warn=True)
    line("스냅샷", "백업\\260301_명단.xlsx 처럼 날짜를 붙여 던져 넣습니다. "
                 "뜨는 시점은 접수 마감 직후, 배정 확정 직후, 발표 자료 낸 직후.")
    line("이 워크북은",
         "명단만 있으면 언제든 다시 만들 수 있습니다. 사본으로 치지 않아도 되고 지워도 됩니다.")
    gap()

    head("학년도와 회계연도는 다릅니다")
    line("유학 업무", "학년도 — 3월 ~ 다음해 2월")
    line("예산", "회계연도 — 1월 ~ 12월")
    line("어긋나는 지점",
         "2026학년도 2학기(9월~2월) 체제비는 9~12월분이 2026회계연도, 1~2월분이 2027회계연도로 갈립니다. "
         "한 학기 예산이 두 회계연도에 걸칩니다. 그래서 예산 서류를 학년도 폴더에 넣으면 "
         "결산 때 반드시 못 찾습니다.", warn=True)
    gap()

    head("제출자료를 왜 분야별로 흩지 않나")
    line("찾는 방식이 다름",
         "제출자료는 '무엇에 관한 자료냐'가 아니라 '언제 누구에게 냈냐'로 기억됩니다. "
         "감사·행감 때 '작년에 뭐 냈지'로 찾지, '예산 관련 제출자료'로 찾지 않습니다.")
    line("묶음이 찢어짐",
         "제출 한 건이 여러 분야에 걸칩니다. 행정사무감사 요구자료 하나에 예산·현황·법령 근거가 다 "
         "들어갑니다. 분야별로 흩으면 그 묶음이 찢어져 '그때 낸 자료 그대로' 재활용을 못 합니다.")
    line("제출은 사건",
         "요구 공문 → 작성 → 검토 → 발송 공문이 한 세트로 남아야 합니다. 그래서 폴더 하나에 넣습니다.")
    line("분야로도 찾으려면",
         "폴더 이름에 분야 낱말을 넣으세요 — 260420_도의회_행감_예산현황. "
         "그러면 탐색기 검색창에 '예산'만 쳐도 잡힙니다. 바로 가기를 만드는 것보다 손이 덜 갑니다.")
    gap()

    head("검토자료를 왜 따로 두나")
    line("섞이면 판단이 늘어남",
         "확정 문서와 초안이 한 폴더에 있으면 파일을 열 때마다 '이게 확정본인가'를 판단해야 합니다. "
         "물리적으로 갈라 두면 그 판단이 없어집니다 — 30_모집 안은 전부 확정·공식 문서입니다.")
    line("70_검토 안쪽",
         "제도개선(지침·조례 개정 검토, 타시도 사례) / 예산검토(단가 시뮬레이션, 소요 추계) / "
         "현황분석(돌려본 표, 보고용 요약) / 질의응답(학교·학부모 문의와 답변 근거)")
    line("끝나면",
         "검토가 확정되어 공문이 나가면 결과물은 해당 업무 폴더로 옮기고, 검토 과정은 70_검토 에 남깁니다. "
         "왜 그렇게 정했는지를 나중에 물어보는 사람이 꼭 있습니다.")
    gap()

    head("한 해가 끝나면")
    line("새로 팔 폴더", "30_모집에 다음 학년도, 40_예산에 다음 회계연도, 50_제출자료에 다음 연도.")
    line("치울 것", "90_이전담당자를 1년째 안 열었으면 압축해서 치웁니다.")
...
def sheet_enrollments(ws, data):
    ...
    last_row = write_table(
        ...
        {"이력ID": 8, "학기": 11, "학년도": 8, "학기구분": 8, "학기순번": 8,
         ...
         "원 지역": 10, "원 소속교": 13, "학기 상태": 9, "학기말 재적": 9, "다음 학기": 11,
    ...
    statc = col_of(ENROLL_HEADERS, "학기 상태")
    stayc = col_of(ENROLL_HEADERS, "학기말 재적")
    sidc = col_of(ENROLL_HEADERS, "학생ID")
    idxc = col_of(ENROLL_HEADERS, "학기순번")
    e_sid = rng("유학이력", ENROLL_HEADERS, "학생ID", LIMIT_ENROLL)
    e_idx = rng("유학이력", ENROLL_HEADERS, "학기순번", LIMIT_ENROLL)
    for r in range(2, last_row + 1):
        ...
        # 이 학생이 다음 학기에 어떻게 되는지. 다음 학기에 같은 학생 줄이 있으면 '이어짐'.
        # 아직 연장 입력이 안 된 마지막 학기는 '학기말 종료' 로 단정하지 않고 '미정' 으로 둔다.
        c = ws.cell(r, ENROLL_HEADERS.index("다음 학기") + 1,
                    f'=IF(${statc}{r}="예정","예정",'
                    f'IF(${stayc}{r}="X","학기 중 종료",'
                    f'IF(COUNTIFS({e_sid},${sidc}{r},{e_idx},${idxc}{r}+1)>0,"이어짐",'
                    f'IF(${statc}{r}="종료","학기말 종료","미정"))))')
    ...
    n = col_of(ENROLL_HEADERS, "다음 학기")
    for value, fill, color in (
        ("이어짐", "D8F0DC", "17652A"),
        ("학기말 종료", "F2F2F2", "595959"),
        ("학기 중 종료", "FCE4E4", "A61C1C"),
        ("미정", "FFF2CC", "7F6000"),
        ("예정", "FFF2CC", "7F6000"),
    ):
        ws.conditional_formatting.add(
            f"{n}2:{n}{LIMIT_ENROLL}",
            CellIsRule(operator="equal", formula=[f'"{value}"'],
                       fill=PatternFill("solid", fgColor=fill),
                       font=Font(name=FONT, size=10, bold=True, color=color)),
        )
...
def sheet_summary(ws, data):
    ...
    n = ws.cell(2, 1,
                "왼쪽부터 '들어온 쪽 → 인원 → 나간 쪽' 순서로 읽습니다. "
                "들어온 쪽: 유학생 수 = 신규 + 계속·재유학. "
                "나간 쪽: 유학생 수 = 이어짐 + 학기말 종료 + 미정·예정 + 학기 중 종료. "
                "'유학생 수(공식 기준)'는 그 학기에 한 번이라도 유학한 인원으로, "
                "도교육청 발표 자료의 시군별 유학생 수와 같은 기준이며 "
                "2022~2025학년도는 13개 시군 숫자까지 모두 일치합니다. "
                "'학기말 재적'은 그 학기 끝까지 남은 인원입니다 "
                "(2026-1학기: 공식 332명 → 학기말 325명).")
    n.font = Font(name=FONT, size=9, color="595959")

    cols = ["학기", "신규", "계속·재유학", "유학생 수(공식 기준)",
            "다음 학기로 이어짐", "학기말 종료", "미정·예정", "학기 중 종료", "학기말 재적",
    ...
    for j, (y, term) in enumerate(sems):
        ...
        put(ws, r, 2, f'=COUNTIFS({E("학기")},{q},{E("구분")},"신규")', bold=True, fmt="#,##0")
        put(ws, r, 3, f'=COUNTIFS({E("학기")},{q},{E("구분")},"<>신규")', fmt="#,##0")
        put(ws, r, 4, f"=COUNTIF({E('학기')},{q})", bold=True, fmt="#,##0")
        put(ws, r, 5, f'=COUNTIFS({E("학기")},{q},{E("다음 학기")},"이어짐")', fmt="#,##0")
        put(ws, r, 6, f'=COUNTIFS({E("학기")},{q},{E("다음 학기")},"학기말 종료")',
            bold=True, fmt="#,##0")
        put(ws, r, 7,
            f'=COUNTIFS({E("학기")},{q},{E("다음 학기")},"미정")'
            f'+COUNTIFS({E("학기")},{q},{E("다음 학기")},"예정")', fmt="#,##0")
        put(ws, r, 8, f'=COUNTIFS({E("학기")},{q},{E("다음 학기")},"학기 중 종료")',
            bold=True, fmt="#,##0")
        put(ws, r, 9, f'=COUNTIFS({E("학기")},{q},{E("학기말 재적")},"O")', fmt="#,##0")
        put(ws, r, 10, f'=COUNTIFS({E("학기")},{q},{E("성별")},"남")', fmt="#,##0")
        put(ws, r, 11, f'=COUNTIFS({E("학기")},{q},{E("성별")},"여")', fmt="#,##0")
        put(ws, r, 12, f'=COUNTIFS({E("학기")},{q},{E("거주 유형")},"가족체류형")', fmt="#,##0")
        put(ws, r, 13, f'=COUNTIFS({E("학기")},{q},{E("거주 유형")},"홈스테이형")', fmt="#,##0")
        put(ws, r, 14, f'=COUNTIFS({E("학기")},{q},{E("거주 유형")},"유학센터형")', fmt="#,##0")
        put(ws, r, 15,
        ...
        put(ws, r, 17, f"=COUNTIF({A('접수 학기')},{q})", fmt="#,##0")
        put(ws, r, 18, f'=COUNTIFS({A("접수 학기")},{q},{A("배정 판정")},"배정")', fmt="#,##0")
        put(ws, r, 19, f'=COUNTIFS({A("접수 학기")},{q},{A("배정 판정")},"<>배정")', fmt="#,##0")
    ...
    # 학년도 요약: 1학기에서 출발해 종료·신규를 더하고 빼며 2학기까지 따라간다
    r1 = last + 4
    t2 = ws.cell(r1, 1, "학년도별 유학생 수 — 1학기에서 2학기까지 사람이 어떻게 드나들었나")
    t2.font = Font(name=FONT, size=12, bold=True, color=INK)
    n2 = ws.cell(r1 + 1, 1,
                 "왼쪽에서 오른쪽으로 한 칸씩 더하고 빼면 그대로 이어집니다. "
                 "1학기 인원에서 종료한 학생을 빼면 2학기로 이어진 인원이 되고, "
                 "거기에 2학기 신규를 더하면 2학기 인원이 됩니다. "
                 "맨 오른쪽 '학년도 인원'은 1학기 인원 + 2학기 신규입니다 — "
                 "두 학기를 다 다닌 학생('2학기로 이어짐')을 두 번 세지 않기 때문에 "
                 "1학기와 2학기를 그냥 더한 값보다 작습니다. "
                 "도교육청 발표 자료의 학년도 수치도 이 기준입니다.")
    n2.font = Font(name=FONT, size=9, color="595959")
    years = sorted({y for y, _ in sems})
    # 머리글은 '=' 나 '+' 로 시작하면 안 된다 — 엑셀이 수식으로 읽는다
    hdr = ["학년도", "1학기 인원", "− 1학기에 종료", "− 1학기 미정", "→ 2학기로 이어짐",
           "＋ 2학기 신규", "→ 2학기 인원", "− 2학기에 종료", "− 2학기 미정",
           "→ 다음 학년도로", "학년도 인원", "셈법", "남", "여", "비고"]
    widths2 = (10, 11, 12, 11, 13, 11, 11, 12, 11, 13, 11, 32, 7, 7, 46)
    for i, h in enumerate(hdr, start=1):
        style_head(ws, r1 + 2, i, h, fill=ACCENT, width=widths2[i - 1])
    ws.row_dimensions[r1 + 2].height = 32
    notes = {
        2022: "발표 자료 27명과 일치 (시범사업 기간, 도청 체제비 미지원). 1학기 모집 없음",
        2023: "발표 자료 85명과 일치. 2학기 모집이 없어 신규 0명 → 1학기 인원이 곧 학년도 인원",
        2024: "발표 자료 165명과 일치",
        2025: "발표 자료 269명과 일치",
        2026: "발표 자료는 1학기 모집 기준 333명 — 고창 1명은 학교 오보고분이며 실제 332명. "
              "1학기 325명은 연장 입력 전이라 '미정'이며, 종료 인원이 아닙니다",
    }
    ...
    for j, y in enumerate(years):
        ...
        def ended(q):
            return (f'=COUNTIFS({E("학기")},{q},{E("다음 학기")},"학기 중 종료")'
                    f'+COUNTIFS({E("학기")},{q},{E("다음 학기")},"학기말 종료")')

        def pending(q):
            return (f'=COUNTIFS({E("학기")},{q},{E("다음 학기")},"미정")'
                    f'+COUNTIFS({E("학기")},{q},{E("다음 학기")},"예정")')

        put(ws, r, 1, f"{y}학년도", bold=True, fill=SUB_FILL)
        put(ws, r, 2, f"=COUNTIF({E('학기')},{q1})", bold=True, fmt="#,##0")
        put(ws, r, 3, ended(q1), fmt="#,##0", color=OUT_C)
        put(ws, r, 4, pending(q1), fmt="#,##0", color=HOLD_C)
        put(ws, r, 5, f'=COUNTIFS({E("학기")},{q1},{E("다음 학기")},"이어짐")', fmt="#,##0")
        put(ws, r, 6, f'=COUNTIFS({E("학기")},{q2},{E("구분")},"<>계속")',
            fmt="#,##0", color=IN_C)
        put(ws, r, 7, f"=COUNTIF({E('학기')},{q2})", bold=True, fmt="#,##0")
        put(ws, r, 8, ended(q2), fmt="#,##0", color=OUT_C)
        put(ws, r, 9, pending(q2), fmt="#,##0", color=HOLD_C)
        put(ws, r, 10, f'=COUNTIFS({E("학기")},{q2},{E("다음 학기")},"이어짐")', fmt="#,##0")
        put(ws, r, 11, f"=$B{r}+$F{r}", bold=True, fmt="#,##0", fill=SUB_FILL)
        put(ws, r, 12,
            f'="1학기 "&$B{r}&"명 + 2학기 신규 "&$F{r}&"명 = "&$K{r}&"명"',
        ...
        for k, g in enumerate(("남", "여")):
            put(ws, r, 13 + k,
                ...
                fmt="#,##0")
        put(ws, r, 15, notes.get(y, ""), align="left", size=9, color="595959")
        ws.row_dimensions[r].height = 15 + 13 * (len(notes.get(y, "")) // 46)
...
def main(src, out):
    ...
    sheet_files(wb.create_sheet("파일정리"))
